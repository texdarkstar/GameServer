...
class CmdPerform(Command):
    ...
    def func(self):
        ...
        if re.match(r"^(\w+)$", args):  # checking for something like 'perform str'
            m = re.match(r"^(\w+)$", args)
        ...
        elif re.match(r"^(\w+) +?\+ +?(\w+)$", args):  # checking for something like 'perform broker + int'
        ...
        elif re.match(r"^(.+\(.+)\) +?\+ +?(\w+)$", args):  # checking for something like 'perform pilot(spacecraft) + dex'
            m = re.match(r"^(.+\(.+)\) +?\+ +?(\w+)$", args)
            string = "You rolled a |c{result}|n: dice rolled[|c{die1}|n, |c{die2}|n] {pm1}{skillname}[|c{skilldm}|n] {pm2}{attr}[|c{attrdm}|n]"
            skillname, attr = m.groups()
